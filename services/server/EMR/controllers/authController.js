...
exports.login = asyncHandler(async (req, res, next) => {
    ...
    if (user) {
        isEqual = await comparePasswords(req.body.password, user.password);
    }
...
exports.protect = asyncHandler(async (req, res, next) => {
    ...
    if (!currentAdmin) {
        return next(
            new ApiError(
                "Admin that belong to this token does no longer exist",
                401
            )
        );
    }
    ...
    if (currentAdmin.passwordChangedAt) {
        ...
        if (passwordChangedTimeStamp > decoded.iat) {
            return next(
                new ApiError(
                    "Admin has recently changed his password. Please login again.."
                ),
                401
            );
        }
...
exports.forgetPassword = asyncHandler(async (req, res, next) => {
    ...
    if (!admin) {
        return next(
            new ApiError(
                `There is no admin with that email: ${req.body.email}`,
                404
            )
        );
    }
    ...
    const message = `<p>Dear ${admin.email.split("@")[0].split(".")[0]},</p>
    ...
    res.status(200).json({
        status: "success",
        message: "Reset code is sent via email",
    });
});
