...
export const createPrescriptionForAppointment = createAsyncThunk(
    ...
    async (newPrescription: Prescription) => {
        try {
            await axios.post(`http://localhost:4001/prescription`, {
...
const prescriptionSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            .addCase(
                createPrescriptionForAppointment.pending,
                (state, action) => {
                    state.loading = true;
                }
            )
            .addCase(
                createPrescriptionForAppointment.fulfilled,
                (state, action) => {
                    state.loading = false;
            ...
            .addCase(
                createPrescriptionForAppointment.rejected,
                (state, action) => {
                    state.loading = false;
                    state.error =
